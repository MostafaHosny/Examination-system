...
namespace OnlineExaminationSystem.Areas.InstructorArea.Controllers
{
    ...
    public class ChaptersController : InstructorBaseController
    {
        ...
        public ActionResult Delete(string id)
        {
                ...
                Chapter chapter = db.Chapters.Find(id);
                //var question = db.Answers.Where(e => e.Question.ChapterId == id).ToList();
                var regId = chapter.RegistrationId;
                var Questions = db.Questions.Where(e => e.ChapterId == id).ToList();
                try
                {
                for (int I = 0; I < Questions.Count; I++)
                {
                    string QuestionID = Questions[I].QuestionId;
                    var answers = db.Answers.Where(e => e.QuestionId == QuestionID).ToList();
